...
description: |
  ...
  使用例:「タスクを最新化して」「今日のゴールを設定して」「日次ふりかえりをしたい」
  「プロジェクトの状況を確認して」「データソースを追加して」「プロジェクトを追加して」
  「タスクのステータスを変更して」など、タスク管理リポジトリ（~/.local/share/my-tasks/）
  への操作を依頼された場合に使用。
---
...
7. **日次ふりかえり** - タスクを最新化し、完了/未完了を確認して振り返りを記録
8. **タスク操作（データソース側）** - ステータス変更・担当者変更・新規作成等をデータソースに反映
9. **参照系** - プロジェクト状況確認・日次ゴール確認・タスク検索
...
- **ステータスの Source of Truth はデータソース側**。リポジトリ側でステータスを直接変更してはならない
- ステータスを変更したい場合は操作8（タスク操作）を使い、データソース側を更新してからタスク最新化で反映する
